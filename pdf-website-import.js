...
function decodeWebsiteBuilderABCBlock(text) {
...
function extractWebsiteBuilderGeneratedTunes(rawText) {
    ...
    // Websites generated by the abcjs-eskin Website Builder embed each tune as:
    // <script type="text/plain" id="some-tune-key-abc"> ... ABC ... </script>
    // Match the script regardless of attribute order, as long as it is a text/plain
    // script block whose id ends with "-abc".
    var blockRegex = /<script\b(?=[^>]*\btype\s*=\s*["']text\/plain["'])(?=[^>]*\bid\s*=\s*["'][^"']+-abc["'])[^>]*>([\s\S]*?)<\/script>/gi;
    var match;

    while ((match = blockRegex.exec(rawText)) !== null) {

        var abcText = decodeWebsiteBuilderABCBlock(match[1]);

        if (abcText && (abcText.indexOf("X:") !== -1)) {
            tunes.push(abcText);
        }
    }
...
function extractLegacyWebsiteTunesArray(rawText) {
...
function extractABCTunesFromShareLinks(links) {
...
function doExtractWebsiteTunes(rawText,filename){

    var extractedTunes = [];

    // First try the abcjs-eskin Website Builder format. It embeds full ABC text
    // directly in text/plain script blocks instead of using share-link URLs.
    extractedTunes = extractWebsiteBuilderGeneratedTunes(rawText);

    // If no Website Builder ABC blocks were found, fall back to the older
    // generated website format that contains const tunes=[{Name,URL}, ...].
    if (!extractedTunes.length) {

        var tunes = extractLegacyWebsiteTunesArray(rawText);
        var links = [];
        ...
        extractedTunes = extractABCTunesFromShareLinks(links);
    }

    var nFound = extractedTunes.length;

    var outVal = "";

    if (nFound > 0) {
        extractedTunes.forEach(abcText => {
            outVal += `${abcText}\n\n`;
        });
    }
...
function extractWebsiteTunes(file){
    ...
    reader.onload = async function() {
        ...
        doExtractWebsiteTunes(rawText,file.name);
...
//
// Import tunes from a local website
//
...
//
// Import tunes from a remote website
//

function getLastRemoteWebsiteHtmlPart(url) {
  const match = url.match(/\/([^\/?#]+\.html)(?:[?#].*)?$/);
  return match ? match[1] : null;
}

function doRemoteWebsiteImport() {

    DayPilot.Modal.prompt("Enter the remote tunebook website URL:", "", {
        theme: "modal_flat",
        top: 200,
        autoFocus: false,
        ...
        try {
            // Wrapping fetch() call to catch sync errors like malformed URLs
            let fetchPromise = fetch(theURL, { method: 'GET' });
            ...
            fetchPromise
                ...
                .then(rawText => {
                    ...
                    setTimeout(function () {
                        ...
                        var filename = getLastRemoteWebsiteHtmlPart(theURL);

                        if (!filename) {
                            filename = "the remote tunebook website";
                        }

                        doExtractWebsiteTunes(rawText, filename);
                ...
                .catch(err => {
                    ...
                    var thePrompt = "There was an issue reading the remote tunebook website.";
            ...
            // Catches sync exceptions like malformed URLs
            hideTheSpinner();
...
function importRemoteWebsiteClickHandler(){
...
function ImportPDF_CSV_Website(){
    ...
    modal_msg += '<p style="text-align:center;margin-top:22px;"><input id="import_website_local" class="btn btn-subdialog import_website" onclick="importWebsiteClickHandler()" type="button" value="Import Tunes from a Website (local)" title="Import tunes from a previously exported ABC Transcription Tools website HTML file stored locally"><input id="import_website_remote" class="btn btn-subdialog import_website" onclick="importRemoteWebsiteClickHandler()" type="button" value="Import Tunes from a Website (remote)" title="Import tunes from a previously exported ABC Transcription Tools website HTML file stored at a remote URL"></p>\n';
